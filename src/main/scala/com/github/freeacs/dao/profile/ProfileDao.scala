...
class ProfileDao(val config: DatabaseConfig[JdbcProfile])(implicit ec: ExecutionContext)
  extends Dao with ProfileTable with ProfileParameterTable {
  ...
  def getByNameAndUnitType(name: String, unitTypeId: Long): Future[Option[(Profile, Seq[ProfileParameter])]] =
    for {
      profile <- db.run(profiles.filter(p => p.profileName === name && p.unitTypeId === unitTypeId).result.headOption)
      params <- {
