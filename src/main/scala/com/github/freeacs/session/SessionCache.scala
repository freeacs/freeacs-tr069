...
class SessionCache extends Actor with ActorLogging {
  ...
  implicit private[this] val cluster = Cluster(context.system)
...
object SessionCache {
  ...
  final private case class Request(key: String, replyTo: ActorRef)
}

class SessionExpiration(key: String, expirationTime: FiniteDuration)
    extends Actor
    with ActorLogging {
  import SessionExpiration._
  import com.github.freeacs.session.SessionCache.Evict
...
object SessionExpiration {
  def props(key: String, expirationTime: FiniteDuration) =
    Props(new SessionExpiration(key, expirationTime))
